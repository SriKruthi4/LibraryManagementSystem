...
app.post('/search', (req, res) => {
  ...
  // Implement the SQL query based on the provided search criteria
  const query = `
...
app.post('/checkout', async (req, res) => {
    ...
    const availabilityResult = await new Promise((resolve, reject) => {
      ...
      if (activeLoansCount > 2 ) {
        ...
        return res.status(400).json({ message: 'Maximum active book loans reached for this borrower.' });

      } else if(available > 0){
        return res.status(400).json({ message: 'Book Not available to checkout.' });
      }else {
...
app.post('/confirmcheckin', (req, res) => {
  ...
  app.get('/fines', (req, res) => {
    ...
    let query = 'SELECT b.card_id, SUM(fine.fine_amt) as total_fine FROM fines fine JOIN book_loans bl ON fine.loan_id = bl.loan_id JOIN borrower b ON bl.card_id = b.card_id where paid = 0 AND fine.fine_amt>0 group by b.card_id';
  ...
  app.put('/updatefines', (req, res) => {
    ...
    CASE
     ...
     WHEN DATEDIFF(bl.date_in,bl.due_date) * 0.25 < 0 THEN 0
